...
import { ArrayHelper, CancelToken, IAnyInstanceType, 
    IDisposable, ignoreValue, INotifyPropertyChanged, PathList } from "./types.js";
...
import { BindableProperty } from "./BindableProperty.js";
...
const setEvent = (name: string) =>
    (ctrl: AtomControl, e: HTMLElement, value: any) => {
...
export class AtomControl implements
    ...
    public onPropertyChanged(name: keyof AtomControl): void {
        switch(name) {
            case "renderer":
    ...
    protected render(node: XNode, e: any = this.element, creator: any = this.creator || this): void {
        ...
        for (const iterator of children) {
            ...
            if (t) {
                throw new Error(`This path is deprecated, check who is calling it.`);
            }
    ...
    protected resolve<TService>(
        ...
        const result = this.app.resolve(c as any, true);
        if (selfName) {
